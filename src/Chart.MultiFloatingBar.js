	...
	Chart.Type.extend({
		...
		initialize:  function(data){
			...
			this.ScaleClass = Chart.Scale.extend({
				...
				calculateBarHeight : function(datasets, dsIndex, barIndex, value) {
					...
					return this.calculateY(datasets[dsIndex].bars[barIndex].endValue - value);
				}
			});
			...
			helpers.each(data.datasets,function(dataset,datasetIndex){
				...
				helpers.each(dataset.data,function(dataPoint,index){
					if (helpers.isNumber(dataPoint.start) && helpers.isNumber(dataPoint.end)){
		...
		addData : function(valuesArray,label){
			...
			helpers.each(valuesArray,function(value,datasetIndex){
				if (helpers.isNumber(value.start) && helpers.isNumber(value.end)){
					//Add a new point for each piece of data, passing any required data to draw.
		...
		draw : function(ease){
			...
			helpers.each(this.datasets,function(dataset,datasetIndex){
				helpers.each(dataset.bars,function(bar,index){
					var y = this.scale.calculateBarY(this.datasets, datasetIndex, index, bar.endValue),
						height = this.scale.calculateBarHeight(this.datasets, datasetIndex, index, bar.startValue);

					//Transition then draw
					bar.transition({
						base: this.scale.calculateY(bar.endValue),
						// base : this.scale.endPoint - (Math.abs(height) - Math.abs(y)),
						x : this.scale.calculateBarX(index),
						y: this.scale.calculateY(bar.startValue),
						// y : Math.abs(y),
